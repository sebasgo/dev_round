defmodule DevRoundWeb.Admin.Fields.BelongsTo do
  @config_schema [
    display_field: [
  ...
  def render_value(assigns) do
    ...
    <div class={[@live_action in [:index, :resource_action] && "truncate"]}>
      <%= if @link do %>
        <.link
          navigate={@link}
          class={[@live_action in [:index, :resource_action] && "truncate", "hover:underline"]}
        >
          {@display_text}
        </.link>
      <% else %>
  ...
  def render_index_form(assigns) do
    %{field_options: field_options, queryable: queryable, display_field_form: display_field_form} =
      assigns
  ...
  def handle_event("update-field", %{"index_form" => %{"value" => value}}, socket) do
    Backpex.Field.handle_index_editable(
      socket,
      value,
      Map.put(%{}, socket.assigns.owner_key, value)
    )
  end
