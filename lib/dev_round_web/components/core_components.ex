defmodule DevRoundWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="bg-neutral/90 fixed inset-0 transition-opacity" aria-hidden="true" />
      <div
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-neutral-700/10 ring-neutral-700/10 relative hidden rounded-2xl bg-primary-content p-14 shadow-lg ring-1 transition"
            >
              <div class="absolute top-6 right-5">
                <button
                  phx-click={JS.exec("data-cancel", to: "##{@id}")}
                  type="button"
                  class="-m-3 flex-none p-3 opacity-50 hover:opacity-90"
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{"primary" => "btn-primary", nil => ""}
  ...
  def header(assigns) do
    ~H"""
    <header class={["py-12 text-center", @class]}>
      <div
        :if={@actions != []}
        class="grid grid-cols-1 items-center justify-items-stretch mx-auto gap-4 md:w-full sm:flex sm:justify-end sm:gap-2 mb-16"
      >
        {render_slot(@actions)}
      </div>
      <h1 class="text-5xl font-mono font-semibold">
        {render_slot(@inner_block)}
      </h1>
      <p :if={@subtitle != []} class="mt-6 text-lg text-base-content/70">
  ...
  def show(js \\ %JS{}, selector) do
    JS.show(js,
      ...
      time: 300,
      transition:
        {"transition-all ease-out duration-300", "opacity-0  sm:scale-95",
  ...
  def hide(js \\ %JS{}, selector) do
    JS.hide(js,
      ...
      time: 200,
      transition:
        {"transition-all ease-in duration-200", "opacity-100 sm:scale-100",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DevRoundWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DevRoundWeb.Gettext, "errors", msg, opts)
    end
  end
